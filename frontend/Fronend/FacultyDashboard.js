...
import { subjectAPI, attendanceAPI, assignmentAPI } from '../services/api';
...
const FacultyDashboard = () => {
  ...
  const fetchStudentsForSubject = async (subjectId) => {
    try {
      const response = await attendanceAPI.getBySubject(
        subjectId,
      ...
      response.data.forEach(record => {
        if (!studentIds.has(record.student._id)) {
      ...
      setAttendanceRecords(uniqueStudents.map(student => ({
