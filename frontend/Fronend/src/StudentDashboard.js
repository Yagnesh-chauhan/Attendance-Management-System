...
const StudentDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [enrolledRes, availableRes, attendanceRes, assignmentsRes, submissionsRes] = await Promise.all([
  ...
  const submitAssignment = async (assignmentId) => {
    try {
      ...
      if (!content && files.length === 0) {
        setError('Please write a submission or upload a file.');
        return;
      }
                ...
                <div className="assignments-list">
                  {assignments.map(assignment => {
                    const mySubmission = mySubmissions.find(
                      (sub) => sub.assignment && sub.assignment._id === assignment._id
                    );
                    ...
                    <div key={assignment._id} className="assignment-card">
                      ...
                      {mySubmission ? (
                        <div className="submission-status" style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '5px' }}>
                          <h4 style={{ color: '#166534', margin: '0 0 10px 0' }}>✅ Submitted</h4>
                          <p><strong>Status:</strong> {mySubmission.status || 'Pending'}</p>
                          {mySubmission.marksObtained !== undefined && (
                            <p><strong>Marks:</strong> {mySubmission.marksObtained}/{assignment.maxMarks}</p>
                          )}
                          {mySubmission.content && (
                            <p><strong>Notes:</strong> {mySubmission.content}</p>
                          )}
                        ...
                        <div className="submission-form" style={{ marginTop: '15px' }}>
                          <textarea
